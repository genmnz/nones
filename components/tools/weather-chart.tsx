...
const WeatherChart: React.FC<WeatherChartProps> = React.memo(({ result }) => {
    ...
    const { chartData, hourlyDataByDay, currentWeather, minTemp, maxTemp, days, airPollution, airPollutionForecast, dailyForecast } = useMemo(() => {
        ...
        const airPollutionForecast = result.air_pollution_forecast?.list?.map((item: AirPollutionData) => {
            return {
                ...item,
        ...
        const dailyForecast = result.daily_forecast?.list?.map((item: DailyForecastData) => {
            return {
                date: new Date(item.dt * 1000).toLocaleDateString(),
        ...
        const groupedData: { [key: string]: WeatherDataPoint } = weatherData.reduce((acc: { [key: string]: WeatherDataPoint }, curr: WeatherDataPoint) => {
            if (!acc[curr.date]) {
                acc[curr.date] = { ...curr };
            } else {
        ...
        const hourlyDataByDay: { [key: string]: WeatherDataPoint[] } = weatherData.reduce((acc: { [key: string]: WeatherDataPoint[] }, curr: WeatherDataPoint) => {
            if (!acc[curr.date]) {
                acc[curr.date] = [];
            }
        ...
        // Get sorted days
        const days = Object.keys(hourlyDataByDay).sort((a, b) => 
    ...
    const renderWeatherBadge = (description: string) => {
        const getColorClass = (desc: string) => {
            ...
            if (lowerDesc.includes('rain') || lowerDesc.includes('drizzle')) return "bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300";
            if (lowerDesc.includes('cloud')) return "bg-neutral-100 text-neutral-700 dark:bg-neutral-800 dark:text-neutral-300";
            ...
            if (lowerDesc.includes('thunder') || lowerDesc.includes('storm')) return "bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300";
            if (lowerDesc.includes('mist') || lowerDesc.includes('fog')) return "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300";
        ...
        <Card className="my-2 py-0 shadow-none bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800 gap-0">
            <CardHeader className="py-2 px-3 sm:px-4">
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                    <Badge variant="outline" className="flex items-center gap-1 py-1 px-3 border rounded-full bg-white/50 dark:bg-neutral-800/50">
                        <Thermometer className="h-3 w-3 text-rose-500 dark:text-rose-400" />
                        <span className="font-medium text-neutral-800 dark:text-neutral-200">{currentWeather.humidity}% Humidity</span>
                    </Badge>
                    <Badge variant="outline" className="flex items-center gap-1 py-1 px-3 border rounded-full bg-white/50 dark:bg-neutral-800/50">
            ...
            <CardContent className="p-0">
                <Tabs defaultValue="chart" className="w-full">
                    <TabsList className="mx-2 sm:mx-4">
                        <TabsTrigger value="chart" className="text-[10px] sm:text-xs px-2 sm:px-3">5-Day Overview</TabsTrigger>
                        <TabsTrigger value="detailed" className="text-[10px] sm:text-xs px-2 sm:px-3">Hourly Forecast</TabsTrigger>
                        <TabsTrigger value="daily" className="text-[10px] sm:text-xs px-2 sm:px-3">16-Day Forecast</TabsTrigger>
                        <TabsTrigger value="airquality" className="text-[10px] sm:text-xs px-2 sm:px-3">Air Quality</TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent value="chart" className="pt-2 px-2 sm:px-4 pb-0">
                        ...
                        <div className="flex items-center justify-center gap-4 mb-2">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[9px] sm:text-[10px] text-neutral-600 dark:text-neutral-400">Min Temperature</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                                <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-[oklch(0.696_0.17_162.48)]" />
                                <span className="text-[9px] sm:text-[10px] text-neutral-600 dark:text-neutral-400">Max Temperature</span>
                            </div>
                        </div>
                                ...
                                <div className="h-[180px] sm:h-[200px] mb-4">
                                    <ResponsiveContainer width="100%" height="100%">
                                        ...
                                        >
                                            ...
                                            <XAxis 
                                                dataKey="timestamp" 
                                                tickFormatter={(value) => new Date(value * 1000).toLocaleDateString(undefined, { 
                                                    weekday: 'short', 
                                                    day: 'numeric' 
                                                })}
                                                tick={{ fontSize: 10 }}
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4">
                                        <div className="flex justify-between items-start mb-3">
                                            <div>
                                                <h3 className="text-sm font-medium text-neutral-800 dark:text-neutral-200">Current Air Quality</h3>
                                                <div className="mt-1">
                                                    {(() => {
                                                        const { label, colorClass } = getAirQualityInfo(airPollution.main.aqi);
                                                        return (
                                    ...
                                    <div className="rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4">
                                        <h3 className="text-sm font-medium mb-4 text-neutral-800 dark:text-neutral-200">Air Quality Forecast</h3>
                                            ...
                                            <>
                                                ...
                                                <div className="flex items-center justify-center gap-4 mb-2">
                                                    <div className="flex items-center gap-1.5">
                                                        ...
                                                        <span className="text-[10px] text-neutral-600 dark:text-neutral-400">Air Quality Index</span>
                                                    </div>
                                                </div>
