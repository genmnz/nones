...
const NearbySearchMapView = memo<NearbySearchMapViewProps>(({
  ...
  const normalizedPlaces = React.useMemo(() => 
    places.map(place => ({
      ...
      price_level: typeof place.price_level === 'string' 
        ? place.price_level.length 
        : place.price_level,
  ...
  const normalizedSelectedPlace = React.useMemo(() => {
    ...
    return {
      ...
      price_level: typeof selectedPlace.price_level === 'string' 
        ? selectedPlace.price_level.length 
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            <div className="flex items-center gap-2 text-sm">
              <span>{places.length} {type} found</span>
              {query && (
                <>
        ...
        <div className="relative flex rounded-full bg-white dark:bg-black border border-neutral-200 dark:border-neutral-700 p-0.5 shadow-lg">
          ...
          <div 
            className={cn(
              ...
              viewMode === 'list' 
                ? "left-0.5 right-[calc(50%-1px)]" 
                : "left-[calc(50%-1px)] right-0.5"
            )}
          />
  ...
  return (
    ...
    prevProps.places.every((place, index) => 
      place.place_id === nextProps.places[index]?.place_id
    )
  );
});
